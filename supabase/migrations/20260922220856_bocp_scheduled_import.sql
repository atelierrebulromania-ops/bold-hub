-- Permit the server-only scheduled importer to use the same atomic import path.
-- The exposed RPC still requires either an active administrator or the
-- service_role API key. Never grant this path to anon users.
create or replace function private.import_bocp_online_orders(p_orders jsonb) returns jsonb
...
begin
  if (select auth.role()) is distinct from 'service_role'
    and ((select auth.uid()) is null or (select private.current_role()) is distinct from 'admin'::public.user_role) then
...
create or replace function private.import_bocp_online_orders_guarded(p_orders jsonb) returns jsonb
language plpgsql security definer set search_path = ''
as $$
begin
  if (select auth.role()) is distinct from 'service_role'
...
grant execute on function private.import_bocp_online_orders_guarded(jsonb) to service_role;
grant execute on function public.import_bocp_online_orders(jsonb) to service_role;
